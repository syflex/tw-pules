...
export class ConsistentHash implements IConsistentHash {
  ...
  AddNode(host: string): IConsistentHash {
    ...
    for (let index = 0; index < indexes.length; index++) {
      // place the node in its positions in the ring.
  ...
  Assign(word: string): string {
    if (this.nodes.length === 0) throw new Error('Add nodes first.');
  ...
  NHash(word: string): Array<number> {
    ...
    for (let index = 0; index < algorithms.length; index++) {
      // find position of host for each hash function
      const hash = crypto
        .createHash(algorithms[index])
        .update(word)
        .digest('hex');
      let number = 0;
  ...
  RemoveNode(host: string): boolean {
    if (this.nodes.length === 0) throw new Error('Hash space is empty');

    const keys = this.NHash(host);

    for (let index = 0; index < keys.length; index++) {
      // remove host and it's virtuals from ring
  ...
  private FindNearestNode(index: number): string | null {
    for (let i = index; i <= this.ringSize; i++) {
      ...
      if (i === this.ringSize) i = i % this.ringSize;
    }
